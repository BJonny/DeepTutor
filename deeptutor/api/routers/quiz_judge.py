...
async def websocket_quiz_judge(websocket: WebSocket):
    ...
    requested_language = (data.get("language") or "").strip().lower().replace("_", "-")
    if requested_language in ("pt", "pt-pt"):
        requested_language = "pt-PT"
    if requested_language not in ("zh", "en", "pt-PT"):
